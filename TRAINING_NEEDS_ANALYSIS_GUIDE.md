...
`Skill gaps: 1` means **one required skill is below the required level** for that employee.

Formula used:
- `gap = requiredLevel - currentLevel`
- a skill is counted as a gap only when `gap > 0`
...
1. **Skill gaps**
- For each missing skill, system creates a training recommendation.
...
- Only positive gaps are counted.
