...
const getEmployeeName = (recommendation: TnaRecommendation): string => {
  if (!recommendation.employee || typeof recommendation.employee === "string")
    return "--";
  const fullName = [
...
export default function TnaExecutionPipelinePage() {
  ...
  const [trainingStatusFilter, setTrainingStatusFilter] =
    useState<TrainingFilterStatus>("all");
  ...
  const selectedEmployeeName = selectedRecommendation
    ? getEmployeeName(selectedRecommendation)
    : "";
  const selectedEmployeeId = selectedRecommendation
    ? getEmployeeId(selectedRecommendation)
    : "";
  ...
  const filteredTrainingRows = useMemo(() => {
    ...
    return selectedTrainingRows.filter((row) => {
      const matchesFilter =
        trainingStatusFilter === "all" || row.status === trainingStatusFilter;
  ...
  const runAutoDeployFromTna = async (planner?: AutoCreatePlannerPayload) => {
    ...
    try {
          ...
          success: {
            render: ({ data }) => {
              const responseMessage = (
                data as { message?: unknown } | undefined
              )?.message;
      ...
      const summary = (response as { data?: AutoDeploySummary } | undefined)
        ?.data;
      if (summary) {
  ...
  const batchCreateParams = useMemo(() => {
    ...
    if (selectedRecommendation.jobRole)
      params.set("jobRole", selectedRecommendation.jobRole);
    if (suggestedBatchName) params.set("suggestedBatch", suggestedBatchName);
    if (suggestedTrainings.length > 0)
      params.set(
        "suggestedTraining",
        suggestedTrainings.slice(0, 3).join(", "),
      );
  ...
  return (
    <div className="pt-14 pb-6 px-4 md:px-6 lg:p-6 space-y-6">
      <section className="flex items-start gap-3">
        <button
          type="button"
          onClick={() => navigate(`/${orgCode}/admin/tna?step=recommendations`)}
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900">
            Training Management (from TNA)
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Manage training delivery setup: program, course, batch, schedules,
            and enrollment from TNA recommendations.
          </p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[340px_minmax(0,1fr)] gap-6">
        <aside
          className={`${cardClassName} h-fit xl:sticky xl:top-20 space-y-3`}
        >
          <div>
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-3">
                ...
                {suggestedTrainings.length === 0 ? (
                  <p className="text-sm text-slate-500 mt-2">
                    No generated training recommendations.
                  </p>
                ) : (
                  <>
                      ...
                      <div className="flex flex-col gap-2 border-b border-slate-200 px-3 py-2.5">
                        <div className="grid grid-cols-1 gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                          <input
                            ...
                            onChange={(event) =>
                              setTrainingSearchTerm(event.target.value)
                            }
                          ...
                          <div className="flex flex-nowrap items-center gap-1.5 overflow-x-auto whitespace-nowrap pb-0.5">
                            {(
                              ...
                              const isActive =
                                trainingStatusFilter === filterItem.key;
                              return (
                                <button
                                  key={filterItem.key}
                                  type="button"
                                  onClick={() =>
                                    setTrainingStatusFilter(filterItem.key)
                                  }
                    ...
                    <div className="mt-3">
                      <p className="text-xs text-slate-600">
                        ...
                        trainingProgressSummary.completed ===
                          trainingProgressSummary.total
                ...
                {deploymentCreateMode === "auto_create" ? (
                  <>
                    ...
                    <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-700 space-y-1.5">
                      ...
                      <p>
                        1. Groups employees by the same pending training title.
                      </p>
                      <p>
                        2. Creates or reuses program, course, and batch (no
                        duplicate enrollments).
                      </p>
                      <p>
                        3. Sets deployed training items from Pending to In
                        Progress automatically.
                      </p>
                      <p>
                        4. Re-clicking auto create only processes new pending
                        items.
                      </p>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            ...
                            <span className="font-semibold">
                              {autoDeploySummary.totalGroups || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            Programs Created:{" "}
                            <span className="font-semibold">
                              {autoDeploySummary.programsCreated || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            Courses Created:{" "}
                            <span className="font-semibold">
                              {autoDeploySummary.coursesCreated || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            Batches Created:{" "}
                            <span className="font-semibold">
                              {autoDeploySummary.batchesCreated || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            Batches Updated:{" "}
                            <span className="font-semibold">
                              {autoDeploySummary.batchesUpdated || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                            Enrollments Added:{" "}
                            <span className="font-semibold">
                              {autoDeploySummary.enrollmentsAdded || 0}
                            </span>
                          </div>
                          <div className="rounded-md border border-[color:color-mix(in_srgb,var(--color-primary,#2563eb)_24%,white)] bg-white px-2.5 py-1.5">
                  ...
                  <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-700 space-y-1.5">
                    ...
                    <p>
                      4. Progress updates are tracked back in TNA recommendation
                      status.
                    </p>
                  </div>
                )}
                {deploymentCreateMode === "manual_create" && (
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="rounded-lg border border-slate-200 bg-white p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">
                          2. Create or select Course
                        </p>
                        <p className="text-xs text-slate-600 mt-0.5">
                          Build the actual training course based on TNA
                          recommendations.
                        </p>
                      </div>
                      <Button
                    ...
                    <div className="rounded-lg border border-slate-200 bg-white p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-600 mt-0.5">
                          In batch creation, choose the course, instructor, and
                          schedule.
                        </p>
                      </div>
                      <Button
                        variant="primary"
                        onClick={() => navigate(createBatchPath)}
                      >
                        Create Batch
                      </Button>
                    </div>
                    ...
                    <div className="rounded-lg border border-slate-200 bg-white p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-600 mt-0.5">
                          After creating the batch, open section details and add
