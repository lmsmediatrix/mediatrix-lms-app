...
import { useBulkImportTnaSkills } from "../../hooks/useTna";

interface BulkImportTnaSkillsModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
interface ImportResult {
  successCount: number;
  successList: SkillImportSuccess[];
  errorCount: number;
...
export default function BulkImportTnaSkillsModal({
  isOpen,
  onClose,
}: BulkImportTnaSkillsModalProps) {
  const { mutate: bulkImport, isPending } = useBulkImportTnaSkills();
  const [file, setFile] = useState<File | null>(null);
  const [importErrors, setImportErrors] = useState<ImportError[]>([]);
  const [importSuccess, setImportSuccess] = useState<SkillImportSuccess[]>([]);
  ...
  const downloadSampleTemplate = () => {
    ...
    const sampleRow = "Emergency Care,Rapid response and critical care readiness\n";
    ...
    link.href = url;
    link.download = "tna-skills-import-template.csv";
  ...
  const handleBulkImport = () => {
    ...
    const importPromise = new Promise<SkillImportResponse>((resolve, reject) => {
      bulkImport(formData, {
        onSuccess: (response: SkillImportResponse) => {
          ...
          if (result.errorCount > 0) {
            setTimeout(() => {
              toast.warning(
                `Import finished: ${result.successCount} processed, ${result.errorCount} row errors`,
              );
            }, 100);
          }
    ...
    void toast.promise(importPromise, {
      pending: "Importing skills...",
      success: "Skills import completed",
      error: {
        render({ data }) {
          const msg =
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Required column:{" "}
                <span className="font-mono text-xs">name</span> (2–100 characters).
              </p>
              <p className="text-xs text-gray-500 mt-2">
                Optional:{" "}
                <span className="font-mono text-xs">description</span> (≤500 characters).
                Duplicate names (case-insensitive) are skipped and counted as already present.
              </p>
            </div>
            <Button
              variant="outline"
        ...
        >
          <div className="text-center">
            ...
            />
            <div className="mb-4">
              <label htmlFor="tna-skills-csv-upload" className="cursor-pointer">
                  ...
                  <div className="bg-green-100 border-b border-green-200 px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-green-800 font-semibold">
                        Processed ({importSuccess.length})
                      </h4>
                    </div>
                  </div>
