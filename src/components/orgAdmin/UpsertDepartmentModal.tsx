      ...
      {isEditMode && isDepartmentLoading ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 md:p-5 space-y-4">
            <div>
              <h3 className="text-base font-semibold text-slate-800">Department Details</h3>
              <p className="text-sm text-slate-500">
                Define the department identity, description, and status.
              </p>
            </div>
            ...
            <div>
              <label
                className={`block text-sm font-medium mb-1 ${
                  errors.isActive ? "text-red-500" : "text-slate-700"
                }`}
              >
                Status
              </label>
              <select
                {...register("isActive")}
                className={`w-full px-3.5 py-2.5 border rounded-xl outline-none transition-colors ${
                  errors.isActive ? "border-red-500" : "border-slate-300 focus:border-primary"
                }`}
                disabled={isLoading}
              >
