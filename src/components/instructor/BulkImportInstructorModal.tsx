...
import { useBulkImportInstructor } from "../../hooks/useInstructor";
import { useAuth } from "../../context/AuthContext";

interface BulkImportInstructorModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
interface InstructorImportResponse {
  message: string;
  result: ImportResult;
}

const BulkImportInstructorModal = ({
  isOpen,
  onClose,
}: BulkImportInstructorModalProps) => {
  const { mutate: bulkImport, isPending } = useBulkImportInstructor();
  ...
  const handleBulkImport = () => {
    ...
    const importPromise = new Promise((resolve, reject) => {
      bulkImport(formData, {
        onSuccess: (response: InstructorImportResponse) => {
    ...
    toast.promise(importPromise, {
      pending: "Importing instructors...",
      success: "Instructors imported successfully",
      error: {
        render({ data }) {
          return (
            (data as { message: string }).message ||
            "Failed to import instructors"
          );
        },
      },
    });
  ...
  const downloadSampleTemplate = () => {
    const templateUrl =
      orgType === "school"
        ? "https://res.cloudinary.com/dyal0wstg/raw/upload/v1749452142/IMPORT-INSTRUCTORS-TEMPLATE_bmk9sj_wyzahw.csv"
        : "https://res.cloudinary.com/dyal0wstg/raw/upload/v1748420228/IMPORT-CORPORATE-INSTRUCTORS-TEMPLATE_afih17.csv";
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Upload a CSV file containing instructor information
              </p>
            </div>
            <Button
              variant="outline"
                ...
                <div className="border-2 border-green-200 rounded-lg bg-green-50 overflow-hidden">
                  <div className="bg-green-100 border-b border-green-200 px-4 py-3">
                    ...
                    <p className="text-sm text-green-700 mt-1">
                      The following instructors were imported successfully:
                    </p>
                  </div>
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-green-200">
                        {importSuccess.map((instructor, index) => (
...
export default BulkImportInstructorModal;
